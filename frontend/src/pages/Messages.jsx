...
import { 
  ...
  Phone, Video, Info, CheckCheck, Smile, Clock, Star
} from "lucide-react";
...
export function MessagesPage() {
  const [activeChat, setActiveChat] = React.useState(tutors[0].id);
  const currentTutor = tutors.find(t => t.id === activeChat) || tutors[0];
    ...
    <div className="pt-20 h-screen bg-white flex overflow-hidden">
      ...
      <aside className="w-full md:w-80 lg:w-96 border-r border-slate-100 flex flex-col bg-slate-50/50">
        <div className="p-6 border-b border-slate-100 bg-white">
          <h1 className="text-2xl font-extrabold text-slate-900 mb-6">Tin nhắn</h1>
          ...
          {mockChats.map((chat) => (
            <button
              ...
              className={`w-full flex items-center p-4 transition-all border-b border-slate-50/50 ${
                activeChat === chat.id ? "bg-white shadow-sm ring-1 ring-slate-100 border-l-4 border-l-indigo-600" : "hover:bg-slate-100/50"
              }`}
            >
              ...
              <div className="ml-4 flex-1 text-left min-w-0">
                <div className="flex justify-between items-start mb-1">
                  <h3 className="font-bold text-slate-900 truncate pr-4">{chat.name}</h3>
                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider flex-shrink-0">{chat.time}</span>
                </div>
                <div className="flex justify-between items-center">
                  <p className={`text-xs truncate ${chat.unread ? "font-bold text-slate-900" : "text-slate-500 font-medium"}`}>
                    {chat.lastMsg}
                  </p>
                  {chat.unread > 0 && (
        ...
        <header className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
          <div className="flex items-center space-x-4">
            <div className="w-10 h-10 rounded-xl overflow-hidden shadow-sm">
              <ImageWithFallback src={currentTutor.avatar} alt={currentTutor.name} className="w-full h-full object-cover" />
            </div>
            <div>
              ...
              <p className="text-[10px] font-bold text-emerald-500 uppercase tracking-widest flex items-center">
                <span className="w-1.5 h-1.5 bg-emerald-500 rounded-full mr-2"></span> Đang hoạt động
              </p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-slate-50/30 custom-scrollbar">
          ...
          {messages.map((msg) => (
            <div key={msg.id} className={`flex ${msg.sender === "me" ? "justify-end" : "justify-start"}`}>
              <div className={`max-w-[75%] group ${msg.sender === "me" ? "order-1" : "order-2"}`}>
                <div className={`px-5 py-3.5 rounded-3xl text-sm font-medium shadow-sm relative ${
                  msg.sender === "me" 
                    ? "bg-indigo-600 text-white rounded-tr-none" 
                    : "bg-white text-slate-700 rounded-tl-none border border-slate-100"
                }`}>
                  {msg.text}
                  <div className={`absolute -bottom-5 whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity text-[10px] font-bold text-slate-400 ${
      ...
      <aside className="hidden lg:flex w-80 border-l border-slate-100 flex-col bg-white">
        <div className="p-8 text-center flex-1">
          ...
          <h3 className="text-xl font-extrabold text-slate-900 mb-1">{currentTutor.name}</h3>
          <p className="text-xs font-bold text-indigo-600 uppercase tracking-widest mb-6">Gia sư {currentTutor.subjects[0]}</p>
          ...
          <div className="grid grid-cols-2 gap-4 mb-8">
            <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100">
              ...
              <div className="text-lg font-extrabold text-slate-900 flex items-center justify-center">
                <Star className="h-4 w-4 text-amber-500 fill-amber-500 mr-1.5" /> {currentTutor.rating}
              </div>
            </div>
            <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100">
              <div className="text-[10px] font-bold text-slate-400 uppercase mb-1">Giờ</div>
              <div className="text-lg font-extrabold text-slate-900">${currentTutor.hourlyRate}</div>
            </div>
          </div>

          <div className="space-y-4 text-left">
            <h4 className="text-sm font-bold text-slate-900 px-1">Buổi học sắp tới</h4>
            <div className="p-4 bg-indigo-50 rounded-2xl border border-indigo-100 flex items-start space-x-3">
              <Clock className="h-5 w-5 text-indigo-600 mt-0.5" />
              <div>
                <p className="text-xs font-bold text-indigo-900">Ngày mai, 10:00 SA</p>
                <p className="text-[10px] font-medium text-indigo-600">Buổi học 50 phút • Trực tuyến</p>
              </div>
            </div>
          </div>
        </div>
